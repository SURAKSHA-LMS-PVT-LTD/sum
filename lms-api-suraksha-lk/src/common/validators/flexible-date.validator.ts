...
export class IsFlexibleDateConstraint implements ValidatorConstraintInterface {
  ...
  private convertToYMDFormat(value: any): string | null {
    ...
    // Pattern 2: MM/dd/yyyy or MM-dd-yyyy or MM.dd.yyyy (US format)
    const mdyPattern = /^(\d{1,2})[\/\-\.](\d{1,2})[\/\-\.](\d{4})$/;
    const mdyMatch = dateString.match(mdyPattern);
    if (mdyMatch) {
      // For ambiguous cases, we'll assume dd/MM/yyyy (international format)
      // If you need MM/dd/yyyy, you should specify this explicitly
    ...
    // Pattern 3: yyyy/MM/dd or yyyy-MM-dd or yyyy.MM.dd
    ...
    // Pattern 4: dd MMM yyyy (e.g., 15 Jan 1990)
    const monthNames = {
      'jan': '01', 'january': '01',
      'feb': '02', 'february': '02',
      'mar': '03', 'march': '03',
    ...
    // Pattern 5: ISO Date string (with time)
  ...
  defaultMessage(): string {
    return 'Date must be in a valid format (yyyy-MM-dd, dd/MM/yyyy, dd-MM-yyyy, etc.) and be a valid date';
  }
}

// Decorator for flexible date validation
...
// Combined decorator for date of birth with flexible input and transformation
export function IsFlexibleDateOfBirth(validationOptions?: ValidationOptions) {
  return function (object: Object, propertyName: string) {
    registerDecorator({
      target: object.constructor,
      propertyName: propertyName,
...
export function formatDateToYMD(date: Date): string | null {
  if (!date) return null;
