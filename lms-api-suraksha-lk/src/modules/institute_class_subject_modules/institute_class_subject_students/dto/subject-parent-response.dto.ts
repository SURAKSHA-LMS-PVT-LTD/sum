...
/**
 * Secure parent response DTO for institute class subject parent endpoints
 ...
 */
export class SubjectParentResponseDto {
  @ApiProperty({ 
    example: '123', 
  ...
  constructor(
    ...
    this.subjectId = subject.id || subject.subjectId;
    this.subjectName = subject.name || subject.subjectName || 'Unknown Subject';
  }
}

/**
 * Query DTO for subject parent endpoints with filtering and pagination
 */
export class SubjectParentQueryDto {
  @ApiPropertyOptional({ 
    example: 1, 
  ...
  @ApiPropertyOptional({ 
    example: true, 
    description: 'Include student details in response (when true, returns unique parents with children array)' 
  })
  students?: boolean;
}

/**
 * Paginated response for subject parents
 */
export class PaginatedSubjectParentResponseDto {
  @ApiProperty({ 
    type: [SubjectParentResponseDto], 
    description: 'Array of parent data' 
  })
  data: SubjectParentResponseDto[];
